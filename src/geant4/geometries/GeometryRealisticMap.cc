#include "geant4/geometries/GeometryRealisticMap.hh"
...
GeometryRealisticMap::GeometryRealisticMap(
  const GeometryRealisticMapParams& params
)
  : fParams(params)
{
    std::cerr
    << "[GeometryRealisticMap] Constructor called\n"
    << "  concreteHalfX_m = " << fParams.concreteHalfX_m << "\n"
    << "  frontPbHalfZ_m  = " << fParams.frontPbHalfZ_m << "\n";
}

GeometryResult GeometryRealisticMap::Construct() {
  ...
  {
    // Parameters
    G4double concWallHalfX = fParams.concreteHalfX_m * m;

    // Keep the +X face fixed at the nominal detector interface.
    G4double concWallMaxX = 29.74142 * m;

    G4ThreeVector concWallHalfSize(
      concWallHalfX,
      1.09*m,
      1.14*m
    );

    G4ThreeVector concWallPos(
      concWallMaxX - concWallHalfX,
      -4.26804*m,
      9.33062*m
    );
  ...
  // *** Front Pb trap ***
  {
    ...
    G4double frontPbHalfZ = fParams.frontPbHalfZ_m * m;

    // Keep the +X face fixed at the nominal scintillator interface.
    // Nominally: centerX = 2.25 m, halfZ = 1.25 m,
    // so the +X boundary is at x = 3.50 m.
    G4double frontPbMaxX = 3.50 * m;

    G4ThreeVector frontPbPos(
      frontPbMaxX - frontPbHalfZ,
      -0.34307*m,
      0.75001*m
    );
